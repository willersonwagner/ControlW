...
begin
  ...
  //ShowMessage(IntToStr(query.fieldbyname('cod').AsInteger));
  ...
  if paramGe38 = '2' then
    begin
      Result.quant := ARREDONDA(quanti /100, 2);
      Result.precoTemp := Result.quant;  //valor total
      Result.quant := ARREDONDA(Result.quant / Result.preco, 2); //quantidade aprox
      quanti := Result.quant; // quanti tem  a quantidade aprox

      while true do
        begin
          _ptot := ARREDONDA(quanti * Result.preco, 2);//valor aprox
          //ShowMessage(CurrToStr(Result.precoTemp) + #13 + CurrToStr(_PTOT) + #13 + CurrToStr(Result.preco) + #13 CurrToStr(quanti));
    ...
    begin
      ...
      Result.precoTemp := ARREDONDA(Result.quant * Result.preco, 2);
    end;
end;
...
begin
  ...
  if form1.pgerais.Values['40'] = 'S' then form7.usarBuscaProdutoCodigoSeq := true
...
begin
  ...
  while true do begin
    ...
    if CopyFile(pwidechar(origem), pwidechar(destino), true) then begin
      Result := true;
      exit;
    end;
    sleep(1000);
    cont := cont + 1;
    if cont > 20 then exit;
