...
uses IniFiles, comctrls, sysutils, controls, classes,
 ...
 procedure GravarConfiguracao(certificadoCaminho, certificadoSenha, certificadoNumeroSerie:String;  FinalidadeNFE,  DANFETipo, DANFEFormaEmissao:integer; DANFELogomarca:String;
                             ...
                             const portaBalanca, veloBal, tipoBal : string; serie : String = '1'; qtdViasStr : String = '1');
 ...
 function EnviarCupomEletronico(nota : String; var estado : String; const imprime, dav : boolean;const lerconfig : boolean = true):Boolean; overload;
 ...
 function EnviarCupomEletronico1(nota : String; var Status, xmotivo : string;const tipo : integer;const enviar : boolean; const cliente1 : String; obs1 : String = ''; serie1 : String = '1'; NNF : STRING = ''):Boolean;
...
var
 ...
 indxImpressoraNFE, serie2      : integer;

 glbCFOP,
 glbNuCheque,
 glbNumeroDAV,
 glbNumeroIP, cliente, serie    : string;
 gTipoEmissao   : string;
...
begin
  ...
  try
    DANFE_Rave.Impressora := SetPrinter(indxImpressoraNFE);
         ...
         begin
          ...
          if tem <> -1 then
            begin
              ...
              item.total := item.total + Arredonda(query1.fieldbyname('quant').AsCurrency * query1.fieldbyname('p_venda').AsCurrency, 2);//query1.fieldbyname('total').AsCurrency;
              totalNota  := totalNota + Arredonda(query1.fieldbyname('quant').AsCurrency * query1.fieldbyname('p_venda').AsCurrency, 2);
            ...
            begin
              ...
              item.total   := Arredonda(query1.fieldbyname('quant').AsCurrency * query1.fieldbyname('p_venda').AsCurrency, 2);//query1.fieldbyname('total').AsCurrency;
...
begin
 ...
 try
  ...
  {$ENDIF}
   ...
   ACBrNFe.Configuracoes.Arquivos.PathCan       := pastaControlW + 'NFCe\CANC\';

   ACBrNFe.Configuracoes.Geral.PathSchemas := ExtractFileDir(ParamStr(0)) + '\Schemas';
...
begin
 ...
 try
  ...
  {$ENDIF}
   ...
   ACBrNFe.Configuracoes.Geral.FormaEmissao     := StrToTpEmis(OK,IntToStr(DANFEFormaEmissao+1));
   ACBrNFe.Configuracoes.Geral.PathSalvar       := pastaControlW + 'NFe\RESP\';
     ...
     begin
       try
         ...
         on e:exception do
           begin
             erro1 := e.Message;
             ACBrNFe.NotasFiscais[0].SaveToFile(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\erro-' + IntToStr(ACBrNFe.NotasFiscais[0].NFe.Ide.cNF) + '.xml' );
             Status := 'vali';
             MessageDlg('Falha na Validação da Nota: ' + #13 + e.Message + #13, mtError, [mbOK], 1);
             Result := false;
             exit;
...
function EnviarCupomEletronico(nota : String; var estado : String; const imprime, dav : boolean;const lerconfig : boolean = true):Boolean;
var
 para,
 ssChave,
 NumeroRecibo : string;
...
begin
 ...
 try
  ...
  try
    ...
    if ACBrNFe.Enviar(0, false) then
      begin
        csta := ACBrNFe.WebServices.Retorno.cStat;
      end;
    estado := 'OK';
        ...
        if Contido('DUPLICIDADE DE NF-E', UpperCase(estado)) then
          begin
            ...
            if ACBrNFe.Consultar then
              begin
                csta := ACBrNFe.NotasFiscais[0].NFe.procNFe.cStat;
                estado := 'NFCe Consultada: Cstat: ' + IntToStr(csta);
              end;
          end
        else
          begin
...
var
 Chave, idLote, CNPJ, Protocolo, Justificativa, tmp, tmp1 : string;
 arq1 : TStringList;
begin
 Result := false;
 query1.Close;
 ...
 if MessageDlg('Tem Certeza que Deseja Cancelar a Nota Fiscal: '+numeroNota+' ?',mtConfirmation,[mbYes, mbNo],0)= mrYes then
  begin
   ...
   if ACBrNFe.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat < 200 then
     begin
       {arq1.LoadFromFile(pastaControlW + 'NFCe\EMIT\' + chave + '-nfe.xml');
       //(nome:string; conteudo : string; const texto :string) : String;
       tmp1 := substitui_Nodo('cStat', inttostr(ACBrNFe.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat), arq1.GetText);
       arq1.Clear;
       arq1.Text := tmp1;
       arq1.SaveToFile(pastaControlW + 'NFCe\EMIT\' + chave + '-nfe1.xml');}
       CopyFile(pchar(pastaControlW + 'NFCe\EMIT\' + chave + '-nfe.xml'), pchar(pastaControlW + 'NFCe\CANC\' + chave + '-nfe.xml'), true);
       DANFE.NFeCancelada := true;
       imprimirNfce;
