   ...
   begin
     if ACBrPosPrinter1.Modelo in [ppEscBematech] then
       begin
         ...
         if acc = 0 then
           begin
             MessageDlg('O Sistema Não Conseguiu Inicializar a Porta!'+  #13+
            'Verifique as Configurações de Porta da Impressora.' + #13
...
begin
  ...
  if ServerIsRunning(servidor, 3050) = false then begin
    ShowMessage('Servidor '+servidor+' Não está Acessível: ');
    exit;
  end;
...
begin
  ...
  progresso('Sincronizando Usuários...',1,2,true,false);
  ini := 0;
  atu := 0;
  ...
  while not IBQueryServer1.Eof do
    begin
      ini := ini + 1;
      progresso('Sincronizando Usuários...',ini,fim,false,false);
