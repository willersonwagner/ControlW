...
uses
  ...
  untnfceForm, pcnConversao, TypInfo, JsEdit1, JsEditNumero1, Math, StrUtils;
...
begin
   edtnumeroseriecert.Text := ACBrNFe.SSL.SelecionarCertificado;
end;
...
begin
 ...
 try
  ...
  {$ENDIF}
   ...
   expandirLogo.Checked         := Ini.ReadBool  ('SERVER', 'expandirLogo' , false);
   usarTLS.Checked         := Ini.ReadBool  ('SERVER', 'usarTLS' , false);
...
var
  ...
  arq : TStringList;
begin
  if usarnfe40.Checked then begin
    if FileExists(ExtractFileDir(ParamStr(0)) + '\4.00.txt') = false then begin
      arq := TStringList.Create;
      arq.SaveToFile(ExtractFileDir(ParamStr(0)) + '\4.00.txt');
      arq.Free;
    end;
  end;
  ...
  Ini.WriteBool('SERVER', 'imprimirNomeFantasia' , imprimirNomeFantasia.Checked);
  Ini.WriteBool('SERVER', 'expandirLogo' , expandirLogo.Checked);
