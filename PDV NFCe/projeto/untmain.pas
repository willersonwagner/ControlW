...
begin
  ...
  while not dtmMain.IBQuery1.Eof do
    begin
      mfd.RichEdit1.Lines.Add(CompletaOuRepete(CompletaOuRepete(dtmMain.IBQuery1.fieldbyname('tipo').AsString , '', ' ', 15), formataCurrency(dtmMain.IBQuery1.fieldbyname('valor').AsCurrency), ' ', 39));

      if trim(dtmMain.IBQuery1.fieldbyname('tipo').AsString) = 'SANGRIA' then total := total - dtmMain.IBQuery1.fieldbyname('valor').AsCurrency
       else total    := total    + dtmMain.IBQuery1.fieldbyname('valor').AsCurrency;
      dtmMain.IBQuery1.Next;
    end;
...
begin
  ...
  while not dtmMain.IBQuery1.Eof do
    begin
      if FileExists(buscaPastaNFCe(dtmMain.IBQuery1.fieldbyname('chave').AsString) + dtmMain.IBQuery1.fieldbyname('chave').AsString + '-nfe.xml') then
        begin
          arq.LoadFromFile(buscaPastaNFCe(dtmMain.IBQuery1.fieldbyname('chave').AsString) + dtmMain.IBQuery1.fieldbyname('chave').AsString + '-nfe.xml');
...
begin
  dtmMain.IBQuery1.Close;
  dtmMain.IBQuery1.SQL.Text := 'update nfce set tentativa = 0 where tentativa > 9';
  dtmMain.IBQuery1.ExecSQL;
  dtmMain.IBQuery1.Transaction.Commit;

  ShowMessage('Comando Executado com Sucesso!' + #13 + IntToStr(dtmMain.IBQuery1.RowsAffected) + ' Registros Modificados.');
end;
...
begin
  ...
  ShellExecute(handle,'open',PChar(ExtractFileDir(ParamStr(0)) + '\DavNFCe.exe'), '','',SW_SHOWNORMAL);
end;
