...
uses IniFiles, comctrls, sysutils, controls, classes,
     ComObj,variants,dialogs,ib_storedproc, StdCtrls, ShDocVw, Forms,
...
begin
  with ACBrNFe.NotasFiscais.Items[0].NFe do
    begin
     ...
     Emit.EnderEmit.xCpl    := '';
     Emit.EnderEmit.xBairro := dadosDest.Values['bairro'];
     Emit.EnderEmit.cMun    := StrToInt((dadosDest.Values['cod_mun']));
   ...
   // if qr.FieldByName('TPPESSOA').AsString = 'J' then
   //  begin
    //  Dest.CNPJCPF          := qr.FieldByName('CNPJCLIENTE').AsString;
    //  Dest.IE               := qr.FieldByName('NUINSCESTCLIENTE').AsString;
    // end else
...
begin
  ...
  with ACBrNFe.NotasFiscais.Items[0].NFe do
    begin
     ...
     Ide.cUF       := 14;
     Ide.cMunFG    := StrToInt(dadosEmitente.Values['cod_mun']);
     //Ide.cMunFG    := 1400100;
         ...
         begin
          ...
          if tem <> -1 then
            ...
            begin
              ...
              try
                cb    := true;
               // cb  := funcoes.checaCodbar(dm.IBselect.fieldbyname('codbar').AsString);
              except
              end;

              if CB then
                begin
                  item.codbar := query2.fieldbyname('codbar').AsString;
                end
              else
                begin
                  //item.codbar := DIGEAN('789000' + funcoes.CompletaOuRepete('', dm.IBselect.fieldbyname('cod').AsString ,'0',6));
                end;
...
begin
 ...
 try
  ...
  vAux := nota;
    ...
    /////GERAR NFC-e:
    GerarNFe(nota, vAux, gTipoEmissao,gTipoAmbiente,gUFComerciante, gFinalidadeNFe);
 ...
 if MessageDlg('Tem Certeza que Deseja Cancelar a Nota Fiscal: '+numeroNota+' ?',mtConfirmation,[mbYes, mbNo],0)= mrYes then
  begin
   ...
   if ACBrNFe.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat < 200 then
     begin
       ShowMessage('NF-e Cancelada com Sucesso. Protocolo '+ ACBrNFe.WebServices.Cancelamento.Protocolo);
       Result := true;
     end
   else
     begin
